...
# 获取实时天气信息
def get_weather():
    url = "https://api.caiyunapp.com/v2.6/91EGzpV4OgclrP3T/101.6656,39.2072/weather?dailysteps=3&hourlysteps=48"
    ...
    if weather_data["status"] == "ok":
        ...
        weather_info = f"温度: {temperature}°C\n湿度: {humidity*100}%\n天气: {skycon}\n空气质量指数 (AQI): {aqi}"
        return weather_info
    else:
        return "无法获取天气信息"
...
weather_interface = gr.Interface(
    ...
    title="Weather Query",
    description="Get the current weather information. Check the box to get an audio response."
)
